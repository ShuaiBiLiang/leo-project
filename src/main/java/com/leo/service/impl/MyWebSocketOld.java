...
public class MyWebSocketOld {
    ...
    //concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocketOld对象。
    private static CopyOnWriteArraySet<MyWebSocketOld> webSocketSet = new CopyOnWriteArraySet<MyWebSocketOld>();
    ...
    public void onOpen(Session session) {
        ...
        try {
            if(StringUtils.hasText(MyWebSocketOld.price)){
                LeoMessage leoMessage = new LeoMessage();
                leoMessage.setPrice(MyWebSocketOld.price);
                leoMessage.setEndTime(MyWebSocketOld.priceTime);
                ServerResponse<LeoMessage> response = ServerResponse.createBySuccess("success",leoMessage);
                response.setMsgType(MyWebSocketOld.MSG_TYPE_REFRESH_PRICE);
    ...
    public void onMessage(String message, Session session) {
        try {
            ...
            response.setMsgType(MyWebSocketOld.MSG_TYPE_STOP_LINK);
            MyWebSocketOld.sendMsg(message, new Gson().toJson(response));
...
//            this.closeWebsocket(message);
        ...
        List<String> userNames = webSocketSet.stream().map(MyWebSocketOld::getUserName).collect(Collectors.toList());
        if(userNames!=null){
            System.out.println("当前在线人数："+userNames.size()+"  详细："+userNames);
        }
        this.setUserName(message);
        //群发消息
//        for (MyWebSocketOld item : webSocketSet) {
//            try {
...
//         error.printStackTrace();
    ...
    public static void sendMsg(String userName, String json, boolean sendOther) throws IOException {
        if(!CollectionUtils.isEmpty(webSocketSet)){
            MyWebSocketOld socket = webSocketSet.stream().filter(o->o.getUserName()!=null &&
            ...
            if(sendOther){
                List<MyWebSocketOld> others = webSocketSet.stream().filter(o->o.getUserName()!=null &&
                        !o.getUserName().equals(userName)).collect(Collectors.toList());
                if(!CollectionUtils.isEmpty(others)){
                    for(MyWebSocketOld o :others){
                        if(o!=null){
                            synchronized(o){
    ...
    public boolean equals(Object o) {
        ...
        MyWebSocketOld that = (MyWebSocketOld) o;
